...
const WebThreads = ({
  ...
  useEffect(() => {
    ...
    // Avoid stacked canvases after Fast Refresh / Strict Mode remounts.
    ...
    const loop = (t: number) => {
      ...
      program.uniforms.uSpeed.value = live.speed;
      program.uniforms.uThreadCount.value = Math.round(live.threadCount);
      program.uniforms.uFrequency.value = live.frequency;
      program.uniforms.uSpread.value = live.spread;
      program.uniforms.uTaper.value = live.taper;
      program.uniforms.uPosition.value = live.position;
      program.uniforms.uFanMode.value = FAN_MODE[live.fanMode] ?? 0;
      program.uniforms.uGlow.value = live.glow;
      program.uniforms.uFalloff.value = live.falloff;
      program.uniforms.uThickness.value = live.thickness;
      program.uniforms.uBrightness.value = live.brightness;
      program.uniforms.uOpacity.value = live.opacity;
      program.uniforms.uMirror.value = live.mirror ? 1.0 : 0.0;
      program.uniforms.uShimmer.value = live.shimmer ? 1.0 : 0.0;
      program.uniforms.uGrain.value = live.grain ? 1.0 : 0.0;
      program.uniforms.uGrainIntensity.value = live.grainIntensity;
    ...
    // Re-apply latest props after mount (Strict Mode remounts skip the props effect).
    ...
    (u.uBackgroundColor.value as Float32Array).set(hexToRgb(p.backgroundColor));
    u.uLightMode.value = p.lightMode;
    u.uMouseStrength.value = p.mouseStrength;
    u.uEnableMouse.value = p.mouseInteraction ? 1.0 : 0.0;
    mouseRef.current.enabled = p.mouseInteraction;
    mouseRef.current.strength = p.mouseStrength;
  ...
  useEffect(() => {
    ...
    const bgArr = u.uBackgroundColor.value as Float32Array;
    bgArr[0] = bg[0];
    bgArr[1] = bg[1];
    bgArr[2] = bg[2];
    u.uLightMode.value = p.lightMode;
